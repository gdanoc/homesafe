...
if (isset($_POST['pedido_id']) && isset($_POST['repartidor_id']) && !empty($_POST['repartidor_id'])) {
    ...
    try {
        ...
        $mensaje_usuario = "Su pedido #$pedido_id ha sido asignado al repartidor " . $info['nombre_repartidor'] . " y está en camino.";
        ...
        $mensaje = "Repartidor asignado correctamente al pedido #".$pedido_id;
        $tipo = "success";
        
    } catch (Exception $e) {
        pg_query($conn, "ROLLBACK");
        
        $mensaje = "Error al asignar repartidor: ".$e->getMessage();
        $tipo = "warning";
    }
} else {
    $mensaje = "Parámetros insuficientes. Debe seleccionar un pedido y un repartidor.";
    $tipo = "warning";
}
