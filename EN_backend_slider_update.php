...
if (isset($_POST['id'], $_POST['nombre'])) {
    ...
    if (!$datos_anteriores) {
        echo "Error: There is no previous data.";
        exit;
    }
    ...
    if (pg_query($conn, $query_update)) {
        ...
        if (!$result_img) {
            echo "Error updating the image: " . pg_last_error($conn);
            exit;
        }
        ...
        echo '<p><script>swal({
            ...
            window.location = "EN_frontend_slider_update.php";
        });</script></p>';
        exit();
    } else {
        echo '<script>swal({
...
<head>
    ...
    <title>Modify Image Carousel</title>
    <!-- Custom CSS -->
...
<body>
    ...
    <div id="main-wrapper">
                    ...
                    <ul class="navbar-nav float-left mr-auto">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="d-none d-md-block">Modify Image Carousel<i class="page-tittle"></i></span>
        ...
        <div class="page-wrapper">
            ...
            <!-- ============================================================== -->
            <div class="form">
                <h1 class="login-title">Modify Images</h1>
                <form action="EN_backend_slider_update.php" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?php echo $row['id']; ?>" />
                    <input type="text" name="nombre" class="login-input" placeholder="Image Name" value="<?php echo

                                                                                                                        tr(htmlspecialchars($row['nombre'], ENT_QUOTES, 'UTF-8')); ?>" required />



                    <input type="file" name="imagen" class="login-input" />
                    <?php if (!empty($row['imagen'])): ?>
                        <?php 
                        
                        $imagen_data = function_exists('pg_unescape_bytea') ? pg_unescape_bytea($row['imagen']) : $row['imagen'];
                        ?>  
                        <img src="data:image/jpeg;base64,<?php echo base64_encode($imagen_data); ?>" alt="Imagen" style="max-width: 200px; max-height: 200px;" />
                    <?php endif; ?>
                    <input type="submit" name="submit" value="Modify" class="login-button" />
                </form>
            </div>
            <script>
                ...
                inputImagen.addEventListener('change', function() {
                    ...
                    if (archivo) {
                        if (!tiposPermitidos.includes(archivo.type)) {
                            mensajeError.textContent = 'Only are allowed image files (JPEG, PNG, GIF, WEBP).';
                            inputImagen.value = ''; 
                        } else {
