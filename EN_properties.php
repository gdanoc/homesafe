...
<body>
        ...
        <section class="search-section">
            <div class="search-container">
                <div class="search-input-wrapper">
                    ...
                    <input type="text" id="searchInput" placeholder="Search properties by name..." onkeyup="searchPropertiesLive()" autocomplete="off">
        ...
        <section class="section novi-background section-md text-center">
            <div class="container">
                <h3 class="text-uppercase font-weight-bold wow-outer">
                    <br>
                    <span class="wow slideInDown">Available Properties</span>
                </h3>
                <br></br>
                        ...
                        <?php foreach ($propiedades as $row): ?>
                            <div class="propiedad-card">
                                        ...
                                        <div class="carousel-inner">
                                            <?php foreach ($row['imagenes'] as $idx => $img): ?>
                                                <div class="carousel-item <?php echo $idx === 0 ? 'active' : ''; ?>">
                                                    <a href="EN_single_property.php?id=<?php echo $row['id']; ?>">
                                ...
                                <div class="propiedad-content">
                                    <h4><?php echo htmlspecialchars(tr($row['nombre'])); ?></h4>
                                    <p><?php echo htmlspecialchars(tr($row['descripcion'])); ?></p>
                                    <div class="propiedad-info">
                                        <span>🛏 Rooms: <?php echo htmlspecialchars($row['bedrooms']); ?></span>
                                        <span>🛁 Bathroms: <?php echo htmlspecialchars($row['bathrooms']); ?></span>
                                    </div>
                                    <div class="propiedad-info">
                                        <span>📏 Size: <?php echo htmlspecialchars($row['size']); ?> m²</span>
                                    </div>
                                    <p class="propiedad-price">$<?php echo number_format($row['precio'], 2); ?></p>
                                </div>
                                <form method="post" action="EN_single_property.php">
                                    <div class="propiedad-actions">
                                        <?php if ($row['estado'] == "Vendido"): ?>
                                            <a class="btn-vendido button button-md button-primary button-winona wow slideInDown">Sold out</a>
                                        <?php else: ?>
                                            <input type="hidden" name="id" value="<?php echo $row['id']; ?>">
                                            <button type="submit" class="btn-contactar button button-md button-primary button-winona wow slideInDown">Contact</button>
                                        <?php endif ?>
                                    </div>
                                </form>
                            </div>
            ...
            if (results.length > 0) {
                results.forEach(function(result) {
                    ...
                    resultItem.onclick = function() {
                        window.location.href = 'EN_single_property.php?id=' + result.id;
                    };
                    resultsContainer.appendChild(resultItem);
                });
            } else {
                resultsContainer.innerHTML = '<div class="no-results"><i class="fa fa-search"></i> No properties found</div>';
            }
            resultsContainer.classList.add('show');
        }
    ...
    <script>
        ...
        <path d="M12 3l9 9-1.5 1.5L18 12.5V20a1 1 0 0 1-1 1h-4v-5h-2v5H7a1 1 0 0 1-1-1v-7.5l-1.5 1.5L3 12l9-9z"/>
            ...
            propiedades.forEach(function(prop) {
                if (prop.latitud && prop.longitud) {
                    ...
                    var vendidoMsg = (prop.estado && prop.estado.toLowerCase() === "vendido") ?
                        `<span style="color:#dc3545;font-weight:bold;">SOLD OUT!</span><br>` :
                        "";
            ...
            Price: $${parseFloat(prop.precio).toLocaleString()}<br>
            <a href="EN_single_property.php?id=${prop.id}" style="color:#007bff;font-weight:bold;text-decoration:underline;">View Details</a>`
                    );
                    marker.on('click', function() {
                        window.location.href = `EN_single_property.php?id=${prop.id}`;
                    });
                }
            });
