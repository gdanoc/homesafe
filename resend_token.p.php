...
if (!$result || pg_num_rows($result) === 0) {
    echo json_encode(['success' => false, 'message' => 'No se encontró un registro de verificación para este email']);
    exit();
}
...
if ($created_at < strtotime('-24 hours')) {
    echo json_encode(['success' => false, 'message' => 'El código de verificación ha expirado. Por favor, regístrese nuevamente.']);
    exit();
}
...
if ($resend_count >= 3 && ($now - $last_resend_at) < 1800) {
    echo json_encode(['success' => false, 'message' => 'Has alcanzado el límite de reenvíos. Intenta nuevamente más tarde.']);
    exit();
}
...
try {
    ...
    $mail->Subject = 'Reenvío: Código de verificación para HomeSafe';
    $mail->Body = "
    <html>
    <head>
        <style>
    ...
    <body>
        <div class='container'>
            ...
            <p>Este es un reenvío de tu código de verificación para completar tu registro.</p>
    ...
    echo json_encode(['success' => true, 'message' => 'Código reenviado correctamente. Revisa tu correo.']);
} catch (Exception $e) {
