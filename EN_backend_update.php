...
if (isset($_POST['id'], $_POST['nombre'], $_POST['descripcion'], $_POST['precio'], $_POST['cantidad'])) {
    ...
    if ($nombre === '') {
        $errores[] = "The name of the piece of furniture is mandatory.";
    } elseif (strlen($nombre) < 2) {
        $errores[] = "The name must be at least 2 characters long.";
    } elseif (strlen($nombre) > 50) {
        $errores[] = "The name cannot exceed 50 characters.";
    } elseif (!preg_match('/^[a-zA-ZáéíóúÁÉÍÓÚñÑ0-9\s\-_.,]+$/', $nombre)) {
        $errores[] = "The name contains invalid characters.";
    }

    
    if ($descripcion === '') {
        $errores[] = "The description is required.";
    } elseif (strlen($descripcion) < 10) {
        $errores[] = "The description must be at least 10 characters long.";
    } elseif (strlen($descripcion) > 500) {
        $errores[] = "The description cannot exceed 500 characters.";
    }
    ...
    if ($precio === '') {
        ...
        $errores[] = "The price must be a valid number.";
    } elseif ($precio < 0.01 || $precio > 99999.99) {
        $errores[] = "The price must be between $0.01 and $99,999.99.";
    } elseif (strpos($precio, '.') !== false && strlen(explode('.', $precio)[1]) > 2) {
        $errores[] = "The price cannot have more than 2 decimal places.";
    }

    
    if ($cantidad === '') {
        $errores[] = "The amount is required.";
    } elseif (!ctype_digit($cantidad)) {
        $errores[] = "The amount must be a whole number.";
    } elseif ($cantidad < 1 || $cantidad > 100000) {
        $errores[] = "The amount must be between 1 and 100,000.";
    }
    ...
    if (isset($_FILES['imagen']) && $_FILES['imagen']['size'] > 0) {
        ...
        if ($extension === 'exe' || $extension === 'bat' || $extension === 'sh') {
            $errores[] = "Executable files are not allowed.";
        } else if (!in_array($tipoArchivo, $tipoPermitido)) {
            $errores[] = "Only JPG, PNG, GIF, or WEBP images are allowed.";
        }
    }
    ...
    if (!$datos_anteriores) {
        echo "Error: Previous data not found.";
        exit;
    }
    ...
    if (pg_query($conn, $query_update)) {
        ...
        if (!$result_img) {
            echo "Error executing image update: " . pg_last_error($conn);
            exit;
        }

        echo '<p><script>swal({
            title: "Success",
            text: "Furniture updated and recorded in the history.",
            icon: "success",
            button: "Close",
        }).then(function() {
            window.location = "EN_frontend_update.php";
...
<body>
    ...
    <div id="main-wrapper">
                    ...
                    <ul class="navbar-nav float-left mr-auto">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle" href="#" id="navbarDropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                <span class="d-none d-md-block">Modify Furniture<i class="page-tittle"></i></span>
                    ...
                    <ul class="navbar-nav float-right">
                        ...
                        <li class="nav-item dropdown">
                            <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark pro-pic" href="#"
                                ...
                                Hello, <?php echo htmlspecialchars($_SESSION['email'], ENT_QUOTES, 'UTF-8'); ?> </a>
        ...
        <div class="page-wrapper">
            ...
            <!-- ============================================================== -->
            <div class="form">
                <h1 class="login-title">Modify Furniture</h1>
                <form id="Inputs" action="EN_backend_update.php" method="post" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="<?php echo $row['id']; ?>" />
                    <input type="text" id="nombre" name="nombre" class="login-input" placeholder="Furniture Name" value="<?php echo

                                                                                                                            htmlspecialchars($row['nombre'], ENT_QUOTES, 'UTF-8'); ?>" required />

                    <input type="text" id="descripcion" name="descripcion" class="login-input" placeholder="Description" value="<?php echo

                                                                                                                                htmlspecialchars($row['descripcion'], ENT_QUOTES, 'UTF-8'); ?>" required />

                    <input type="number" id="precio" name="precio" class="login-input" placeholder="Price" value="<?php echo

                                                                                                                    htmlspecialchars($row['precio'], ENT_QUOTES, 'UTF-8'); ?>" required />
                    <input type="number" id="cantidad" name="cantidad" class="login-input" placeholder="Quantity" value="<?php echo

                                                                                                                            htmlspecialchars($row['cantidad'], ENT_QUOTES, 'UTF-8'); ?>" required />
                    <input type="number" id="descuento" name="descuento" class="login-input" placeholder="Discount (%) - Optional" min="0" max="100" step="0.01" value="<?php
                ...
                document.addEventListener("DOMContentLoaded", function() {
                    ...
                    form.addEventListener("submit", function(event) {
                        ...
                        if (nombreInput.value.trim() === "") {
                            isValid = false;
                            errorMessage += "The 'Furniture Name' field cannot be left blank.\n";
                        } else if (nombreInput.value.length > 50) {
                            isValid = false;
                            errorMessage += "The 'Furniture Name' field cannot have more than 50 characters.\n";
                        } else if (!/^[a-zA-Z0-9]+$/.test(nombreInput.value)) {
                            isValid = false;
                            errorMessage += "The 'Funiture Name' field only can have letters and numbers.\n";
                        }
                        ...
                        if (descripcionInput.value.trim() === "") {
                            ...
                            isValid = false;
                            errorMessage += "The 'Description' field cannot have more than 500 characters.\n";
                        } else if (!/^[a-zA-Z0-9\s]+$/.test(descripcionInput.value)) {
                            isValid = false;
                            errorMessage += "The 'Description' field only can have letters, numbers and blank spaces.\n";
                        }

                        
                        if (precioInput.value.trim() === "") {
                            isValid = false;
                            errorMessage += "The 'Price' field cannot be empty.\n";
                        } else if (parseInt(precioInput.value) < 1 || parseInt(precioInput.value) > 9999) {
                            isValid = false;
                            errorMessage += "The 'Precio' field must be between 1 and 9999.\n";
                        }

                        
                        if (cantidadInput.value.trim() === "") {
                            isValid = false;
                            errorMessage += "The 'Quantity' field cannot be empty.\n";
                        } else if (parseInt(cantidadInput.value) < 1 || parseInt(cantidadInput.value) > 100000) {
                            isValid = false;
                            errorMessage += "The 'Quantity' field must be between 1 and 100000.\n";
                        }
                    ...
                    nombreInput.addEventListener("input", function() {
                        ...
                        if (nombreInput.value.trim() === "") {
                            isValid = false;
                            errorMessage += "The 'Furniture Name' field cannot be empty.\n";
                        } else if (nombreInput.value.length > 50) {
                            isValid = false;
                            errorMessage += "The 'Furniture Name' field cannot have more than 50 characters.\n";
                        } else if (!/^[a-zA-Z0-9]+$/.test(nombreInput.value)) {
                            isValid = false;
                            errorMessage += "The 'Furniture Name' field only can have letters and numbers.\n";
                        }
                    ...
                    descripcionInput.addEventListener("input", function() {
                        ...
                        if (descripcionInput.value.trim() === "") {
                            ...
                            isValid = false;
                            errorMessage += "The 'Description' field only can hace letters, numbers and blank spaces.\n";
                        }
                    ...
                    precioInput.addEventListener("input", function() {
                        ...
                        if (precioInput.value.trim() === "") {
                            ...
                            isValid = false;
                            errorMessage += "The 'Price' field must be between 1 and 9999.\n";
                        }
            ...
            <script>
                document.getElementById('imagenInput').addEventListener('change', function() {
                    ...
                    if (!tiposPermitidos.includes(file.type)) {
                        errorMsg.textContent = 'Error: Only image files are allowed. (JPEG, PNG, GIF, WEBP)';
                        input.value = '';
                        newImagePreview.style.display = 'none';
                    ...
                    reader.onload = function(e) {
                        ...
                        img.onerror = function() {
                            errorMsg.textContent = 'Error: The file is not a valid image.';
                            input.value = '';
                            newImagePreview.style.display = 'none';
