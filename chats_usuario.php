...
$query = "SELECT * FROM chats_manager WHERE id_casa IS NOT NULL AND user_gmail_customer = $1 ORDER BY id_chat DESC";
$result_chats = pg_query_params($conn, $query, array($_SESSION['email']));

$chats = [];
while ($row = pg_fetch_assoc($result_chats)) {
    $chats[] = $row;
...
<head>
    <title>Mis Chats - HomeSafe</title>
...
<body>
    <div class="page">
        ...
        <!-- Contenido de Mis Chats -->
        <section class="section novi-background section-md text-center">
            <div class="container">
                <h3 class="text-uppercase font-weight-bold wow-outer">
                    <br>
                    <span class="wow slideInDown">Mis Chats</span>
                </h3>
                <?php if (count($chats) > 0): ?>
                    <div class="chats-container">
                        <?php foreach ($chats as $index => $chat): ?>
                            ...
                            $query2 = "SELECT nombre FROM propiedades WHERE id = $1";
                            $result2 = pg_query_params($conn, $query2, array($chat['id_casa']));
                            $row2 = pg_fetch_assoc($result2);
                            $nombre_casa = $row2['nombre'] ?? 'Sin nombre';
                            ?>  
                            <div class="chat-accordion-item" data-chat-id="<?php echo $chat['id_chat']; ?>">
                                    ...
                                    <div class="chat-summary">
                                        <div class="chat-title">
                                            <h5 class="mb-1">
                                                ...
                                                Chat con: <?php echo htmlspecialchars($chat['user_gmail_other']); ?>
                                            </h5>
                                            <p class="property-name mb-1">
                                                <i class="fas fa-home me-1"></i>
                                                <?php echo $nombre_casa; ?>
                                            </p>
                                        </div>
                                        <div class="chat-status-container">
                                            <?php if ($chat['estado'] == 't'): ?>
                                                <span class="status-badge status-accepted">
                                                    <i class="fas fa-check-circle me-1"></i>
                                                    Aceptado
                                                </span>
                                            <?php elseif ($chat['estado'] == 'f'): ?>
                                        ...
                                        <div class="details-grid">
                                            <div class="detail-item">
                                                ...
                                                <div class="detail-value">
                                                    <?php echo htmlspecialchars($chat['user_gmail_customer']); ?>
                                                </div>
                                            </div>
                                            <div class="detail-item">
                                                <div class="detail-label">
                                                ...
                                                <div class="detail-value">
                                                    <?php echo htmlspecialchars($chat['user_gmail_other']); ?>
                                                </div>
                                            </div>
                                            <div class="detail-item <?php echo ($chat['estado'] == 'f') ? 'detail-item-canceled' : ''; ?>">
                                                <div class="detail-label">
                                                ...
                                                <div class="detail-value">
                                                    <?php if ($chat['estado'] == 't'): ?>
                                                        <span class="text-success">✓ Conversación activa</span>
                                                    <?php elseif ($chat['estado'] == 'p'): ?>
                                                        <span class="text-warning">⏳ Esperando aprobación del vendedor</span>
                                                    <?php elseif ($chat['estado'] == 'f'): ?>
                                                        <span class="text-canceled">❌ Chat Cancelado/Rechazado</span>
                                                    <?php endif; ?>
                                                </div>
                                            </div>
                                        </div>
                                        <div class="chat-actions mt-4">
                                            <?php if ($chat['estado'] == 'p'): ?>
                                                <button class="btn btn-warning btn-action" disabled>
                                                    <i class="fas fa-hourglass-half me-2"></i>
                                                    Esperando aprobación
                                                </button>
                                                <small class="text-muted d-block mt-2">
                                                    El vendedor debe aprobar tu solicitud para iniciar el chat
                                                </small>
                                            <?php elseif ($chat['estado'] == 't'): ?>
                                                <form method="post" action="chat/index.php" style="display: inline;">
                                                    <input type="hidden" name="id_chat" value="<?php echo $chat['id_chat']; ?>">
                                                ...
                                                <small class="text-success d-block mt-2">
                                                    <i class="fas fa-check-circle me-1"></i>
                                                    Chat disponible - ¡Puedes conversar ahora!
                                                </small>
                                            <?php elseif ($chat['estado'] == 'f'): ?>
                                                <div class="canceled-actions">
                                                    ...
                                                    <h6 class="text-danger mb-2">Chat no disponible</h6>
                                                    <p class="text-muted mb-3">
                                                        Este chat ha sido cancelado o rechazado por el vendedor.
                                                        No es posible continuar la conversación.
                                                    </p>
                                                    <button class="btn btn-danger btn-action mb-3" disabled>
                                                        <i class="fas fa-times-circle me-2"></i>
                                                        Chat Cancelado
                                                    </button>
                                                    <div class="alternative-actions">
                                                        <small class="text-muted d-block mb-2">¿Qué puedes hacer?</small>
                                                        <div class="d-flex flex-wrap justify-content-center gap-2">
                                                            <a href="properties.php" class="btn btn-outline-primary btn-sm">
                ...
                <?php else: ?>
                    <div class="empty-chats">
                        ...
                        <p class="text-muted mb-4">Cuando inicies conversación con un vendedor, aparecerá aquí.</p>
