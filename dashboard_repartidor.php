...
if (!isset($_SESSION['user_id'])) {
    ...
    if (pg_num_rows($result) > 0) {
        ...
        echo '<script>          swal({          title: "Error",          text: "No se pudo encontrar tu cuenta de repartidor",          icon: "warning",          button: "Cerrar",          }).then(function() {          window.location = "index.php";          });
        </script>';
        exit();
    }
}
...
<body>
    <header style="background-color: #f5581a; padding: 10px 20px; color: white; display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap;">
        <div style="font-size: 18px; font-weight: bold;">
            Hola, <?php echo htmlspecialchars($_SESSION['email']); ?>
        </div>
        ...
        <script>
            function changeLanguage() {
                ...
                if (lang === 'es') {
                    window.location.href = 'dashboard_repartidor.php';
                } else if (lang === 'en') {
                    window.location.href = 'EN_dashboard_repartidor.php';
                }
            }
        </script>
        <div>
            <form action="logout.p.php" method="post" style="margin: 0;"> <button type="submit" style="          background-color: #fff;          color: #024866ff;          border: none;          padding: 8px 16px;          border-radius: 6px;          font-weight: bold;          cursor: pointer;          transition: background-color 0.3s ease;          " onmouseover="this.style.backgroundColor='#f2f2f2'" onmouseout="this.style.backgroundColor='#fff'"> Cerrar Sesión </button>
            </form>
        </div>
    </header>

    <div class="form">
        <h1 class="login-title">Pedidos Asignados</h1>
            ...
            <div class="table-container">
                <table>
                    ...
                    <tbody> <?php foreach ($pedidos as $pedido): ?> <tr>
                                ...
                                <td> <button class="details-btn" data-pedido='<?php echo json_encode($pedido); ?>'>Ver Detalles</button> </td>
                                <td>
                                    <form method="post" action="chat/chat_repartidor/rep.php" style="display: inline;">
    ...
    <div id="detailsModal" class="modal">
        <div class="modal-content">
            ...
            <button id="btnOpenLocationModal" style="margin-top: 15px; background-color: #28a745; color: white; border: none; padding: 10px 15px; border-radius: 5px; cursor: pointer; font-weight: bold;"> 📍 Seleccionar mi ubicación y ver ruta
            </button>
            ...
            <div id="deliveryControls" class="delivery-controls" style="display: none;">
                ...
                <div class="delivery-status-card">
                    ...
                    <div class="alert alert-info" role="alert">
                        ...
                        <span id="delivery-message">Selecciona tu ubicación para comenzar</span>
                    </div>
                    ...
                    <div class="delivery-info-grid">
                        <div class="info-box">
                            <small>Estado</small>
                            <div class="value" id="delivery-status">Preparando</div>
                        </div>
                        <div class="info-box">
                            <small>ETA</small>
                ...
                <div id="deliveryStatus" class="delivery-status">
                    ...
                    <span id="statusText">Estado: Asignado</span>
                </div>

                <!-- Botones de control -->
                <div id="deliveryButtons">
                    <button id="btnStartDelivery" class="delivery-button"> 🚀 Iniciar Entrega </button>
                    <button id="btnFinishDelivery" class="delivery-button finish" style="display: none;"> ✅ Finalizar Entrega </button>
                </div>

                <!-- ETA Display (legacy - mantenido para compatibilidad) -->
                <div id="etaDisplay" class="eta-display" style="display: none;">
                    ⏱️ Tiempo estimado de llegada: <span id="etaTime">--</span>
                </div>

                <!-- Información adicional (legacy - mantenido para compatibilidad) -->
                <div id="deliveryInfo" class="delivery-info" style="display: none;">
                    <p id="deliveryMessage">Preparando simulación de entrega...</p>
                </div>
            ...
            <div id="rutaPedidoContainer" style="display:none; margin-top:30px;">
                <h3 style="margin-bottom:10px;">Ruta del pedido</h3>
    ...
    <div id="locationModal" class="modal">
        <div class="modal-content">
            ...
            <h2>📍 Seleccionar Ubicación del Repartidor</h2>
            <div class="instruction-message"> <i class="fas fa-info-circle"></i> Haz clic en el mapa para seleccionar tu ubicación actual. Se trazará automáticamente la ruta hasta el destino.
            </div>
            <div id="mapRoute"></div>
            <div id="locationMessage" style="margin-top: 10px; padding: 10px; border-radius: 5px; text-align: center;"></div>
            <button id="btnSaveLocation" class="btn-save-location" disabled> 💾 Guardar Ubicación Seleccionada
            </button>
        </div>
    </div>
    ...
    <script src="delivery_simulator.js"></script>
    ...
    <script>
        ...
        <p><strong>Email Cliente:</strong> ${pedido.email}</p>
        <p><strong>Dirección de Envío:</strong> ${pedido.direccion_envio.replace('<br>')}</p>
        <p><strong>Teléfono:</strong> ${pedido.telefono}</p>
        <p><strong>Estado Pedido:</strong> ${pedido.estado.charAt(0).toUpperCase() + pedido.estado.slice(1)}</p>
        <p><strong>Estado Entrega:</strong> ${pedido.estado_entrega.charAt(0).toUpperCase() + pedido.estado_entrega.slice(1)}</p>
        <p><strong>Fecha de Pedido:</strong> ${new Date(pedido.fecha_pedido).toLocaleString()}</p>
        <h3>Productos:</h3>
    ...
    `;
            ...
            setTimeout(() => {
                if (!map) {
                    ...
                    marker = L.marker([lat, lng]).addTo(map).bindPopup('📍 Destino: ' + pedido.direccion_envio).openPopup();
                } else {
                    map.setView([lat, lng], 15);
                    marker.setLatLng([lat, lng]);
                    marker.bindPopup('📍 Destino: ' + pedido.direccion_envio).openPopup();
                }
                map.invalidateSize();
            }, 100);
            ...
            if (pedido.latitud_repartidor && pedido.longitud_repartidor) {
                ...
                setTimeout(() => {
                    ...
                    }).addTo(window.mapRutaPedido).bindPopup('🏠 Destino').openPopup();
                    ...
                    }).addTo(window.mapRutaPedido).bindPopup('🚚 Tu ubicación').openPopup();
        ...
        function updateDeliveryStatus(estado) {
            ...
            switch (estado.toLowerCase()) {
                case 'asignado':
                    ...
                    statusText.textContent = 'Estado: Asignado';
                    break;
                case 'en camino':
                case 'en_camino':
                    statusIndicator.classList.add('status-en-camino');
                    statusText.textContent = 'Estado: En Camino';
                    break;
                case 'entregado':
                    statusIndicator.classList.add('status-entregado');
                    statusText.textContent = 'Estado: Entregado';
                    break;
                default:
                    statusIndicator.classList.add('status-asignado');
                    statusText.textContent = 'Estado: ' + estado;
            }
        }
        ...
        function startDeliveryProcess(pedido) {
            ...
            btnStartDelivery.textContent = '⏳ Iniciando...';
            ...
            fetch('iniciar_entrega.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        swal({
                            ...
                            text: "La simulación de entrega ha comenzado.",
                            icon: "success",
                            button: "Continuar"
                        });
                    } else {
                        swal({
                            title: "Error",
                            text: data.error || "No se pudo obtener datos actualizados",
                            icon: "error",
                            button: "Cerrar"
                        });
                    }
                ...
                .catch(error => {
                    swal({
                        title: "Error",
                        text: "Error en la comunicación: " + error.message,
                        icon: "error",
                        button: "Cerrar"
                    });
                })
                .finally(() => {
                    ...
                    btnStartDelivery.textContent = '🚀 Iniciar Entrega';
                });
        }
        ...
        function finishDeliveryProcess(pedido) {
            ...
            btnFinishDelivery.textContent = '⏳ Finalizando...';
            ...
            fetch('finalizar_entrega.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        swal({
                            ...
                            text: "El pedido ha sido marcado como entregado exitosamente.",
                            icon: "success",
                            button: "Cerrar"
                        });
                    } else {
                ...
                .finally(() => {
                    ...
                    btnFinishDelivery.textContent = '✅ Finalizar Entrega';
                });
        }

        function initializeDeliverySimulation(pedido) {
            console.log('Inicializando simulación con pedido:', pedido);

            
            if (!pedido.latitud_repartidor || !pedido.longitud_repartidor || !pedido.latitud || !pedido.longitud) {
                console.error('Coordenadas faltantes para la simulación');
                return;
            }
        ...
        function iniciarMapaRuta() {
            ...
            setTimeout(() => {
                ...
                locationMessage.textContent = 'Haz clic en el mapa para seleccionar tu ubicación actual.';
                mapRoute.on('click', function(e) {
                    ...
                    if (markerRepartidor) {
                        ...
                        }).addTo(mapRoute).bindPopup('🚚 Tu ubicación').openPopup();
                    }
                    if (controlRouting) {
                    ...
                    locationMessage.innerHTML = '✅ Ubicación seleccionada correctamente. Ruta trazada. <br><strong>Puedes guardar la ubicación ahora.</strong>';
        ...
        btnSaveLocation.addEventListener('click', () => {
            if (latRepartidor === null || lonRepartidor === null) {
                swal({
                    title: "Error",
                    text: "No se ha seleccionado una ubicación válida.",
                    icon: "warning",
                    button: "Cerrar"
                });
                return;
            ...
            btnSaveLocation.disabled = true;
            btnSaveLocation.textContent = '⏳ Guardando...';
            fetch('update_location.php', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        swal({
                            title: "Error",
                            text: "Error al guardar la ubicación: " + (data.error || 'Error desconocido'),
                            icon: "error",
                            button: "Cerrar"
                        });
                    }
                ...
                .finally(() => {
                    btnSaveLocation.disabled = false;
                    btnSaveLocation.textContent = '💾 Guardar Ubicación Seleccionada';
                });
        });
    </script>
</body>
