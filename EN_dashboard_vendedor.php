<?php
session_start();
include('0_ENvalidacion_Seller.php'); //ESPAÑOL 
include('EN_logic_dashboard_vendedor.php');
...
if ($result_planes) {
    while ($row = pg_fetch_assoc($result_planes)) {
        $planes[$row['id']] = [
            ...
            'duracion' => ($row['id'] == 4) ? '21 days' : '1 month'  
        ];
    }
}
pg_close($conn);
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['pago_paypal']) && $_POST['pago_paypal'] === 'completado') {
    ...
    if (!$plan_id_post || !$paypal_transaction_id) {
        $_SESSION['error_msg'] = "Invalid data to process subscription.";
    } elseif (!preg_match('/^\d{4}-\d{4}$/', $telefono)) {
        $_SESSION['error_msg'] = "Phone number must be in format XXXX-XXXX.";
    } elseif (!in_array($plan_id_post, [1, 2, 3, 4])) {
        $_SESSION['error_msg'] = "Selected plan is not valid. Please select a valid plan.";
    } elseif ($plan_id_post == 4 && $ya_tuvo_trial) {
        $_SESSION['error_msg'] = "The trial plan can only be used once. Please select a paid plan.";
    }
...
<head>
    ...
    <title>CRUD Seller - HomeSafe</title>
    <!-- Custom CSS -->
...
<body>
    ...
    <div id="main-wrapper">
        <header class="topbar" data-navbarbg="skin5">
            <nav class="navbar top-navbar navbar-expand-md navbar-dark">
                <div class="navbar-header" data-logobg="skin5">
                    ...
                    <a class="navbar-brand" href="EN_dashboard_vendedor.php">
                ...
                <div class="navbar-collapse collapse" id="navbarSupportedContent" data-navbarbg="skin5">
                    ...
                    <div class="view-switcher-container">
                        <a href="EN_index.php" class="btn-view-switch" title="Switch to User View">
                            <i class="fas fa-user-circle"></i>
                            <span class="switch-text">User View</span>
                    ...
                    <ul class="navbar-nav float-right">
                        <div class="language-selector">
                            <div>
                                <button id="darkModeToggle" class="language-select">Change Theme</button>
                                <script>
                        ...
                        <li class="nav-item dropdown">
                            <div class="collapse-profile">
                                <a class="nav-link dropdown-toggle text-muted waves-effect waves-dark pro-pic" href="#"
                                    ...
                                    <span class="user-text">Hello, <?php echo htmlspecialchars($username); ?></span>
                                </a>
                                <div class="dropdown-menu dropdown-menu-right user-dd animated">
                                    <a class="dropdown-item" href="EN_logout.p.php"><i class="fa fa-power-off m-r-5 m-l-5"></i> Logout</a>
                                </div>
                            </div>
                        </li>
                    </ul>
        ...
        <?php require("left_sidebar/EN_seller.php"); ?>
        <!-- ============================================================== -->
        <!-- Page wrapper -->
        <!-- ============================================================== -->
        <div class="page-wrapper">
                ...
                <section class="section novi-background section-md text-center">
                    <div class="container">
                        <div class="alert alert-danger text-center mb-4" role="alert" style="font-size:1.2em;">
                            ...
                            Renew your subscription to keep using seller features.
                        </div>
                        <div class="payment-instructions">
                            <i class="fas fa-info-circle"></i>
                            <strong>Payment Instructions:</strong> Click the PayPal button to renew your subscription.<br>
                            <span style="font-size:0.95em;color:#666;">
                                If you want to change your plan, you can do it in the <a href="EN_suscripciones.php" style="color:#007bff;text-decoration:underline;">plans section</a>.
                            </span>
                        </div>
                        <h3 class="text-uppercase font-weight-bold wow-outer">
                            <span class="wow slideInDown">Renew Subscription</span>
                        </h3>
                        <div class="row">
                            <div class="col-lg-6">
                                <div class="checkout-container">
                                    ...
                                    <form id="formSuscripcion" method="POST" action="">
                                        ...
                                        <div class="payment-instructions">
                                            ...
                                            <strong>Payment Instructions:</strong> Click the PayPal button to renew your subscription.<br>
                                            <span style="font-size:0.95em;color:#666;">
                                                You can also update your plan here by selecting a different one.
                                            </span>
                                        </div>
                                        <div class="mb-2">
                                        ...
                                        <div class="mb-2">
                                            <label for="plan_id_select">Select your plan:</label>
                                            <select class="form-control" id="plan_id_select" name="plan_id" required>
                                                <?php foreach ($planes as $id => $plan): ?>
                                                    <option value="<?= $id ?>" <?= ($suscripcion['plan_id'] == $id) ? 'selected' : '' ?>>
                            ...
                            <div class="col-lg-3">
                                <div class="order-summary">
                                    ...
                                    <div class="subscription-item d-flex align-items-center py-2">
                                        <div class="flex-grow-1">
                                            <div class="plan-highlight">
                                                <h5 id="plan_nombre">
                                                    <?= isset($nombres_planes[$suscripcion['plan_id']]) ? $nombres_planes[$suscripcion['plan_id']] : "Trial" ?> ©
                                                </h5>
                                                <p id="plan_descripcion">
                                                    <?= isset($descripciones_planes[$suscripcion['plan_id']]) ? tr($descripciones_planes[$suscripcion['plan_id']]) : "" ?>
                                                </p>
                                                <div class="price-display" id="plan_pago">Monthly payment</div>
                                            </div>
                                        </div>
                                    </div>
                                    <hr>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Subtotal:</span>
                                        <span id="plan_precio">$<?= number_format($precios_planes[$suscripcion['plan_id']], 2) ?></span>
                                    </div>
                                    <div class="d-flex justify-content-between mb-2">
                                        <span>Duration:</span>
                                        <span id="plan_duracion"><?= ($suscripcion['plan_id'] == 4) ? "21 days" : "1 month" ?></span>
                                    </div>
                                    <hr>
                                    <div class="d-flex justify-content-between mb-0">
                                        <strong>Total:</strong>
                                        <strong class="fs-5" id="plan_total">$<?= number_format($precios_planes[$suscripcion['plan_id']], 2) ?></strong>
                                    </div>
                                    <div class="mt-3">
                                        <small class="text-muted">
                                            <i class="fas fa-shield-alt me-1"></i> Secure payment with PayPal
                                        </small>
                                    </div>
                                </div>
                            </div>
            ...
            <div class="form">
                <h1 class="login-title">Property List</h1>
                        ...
                        <tbody>
                            ...
                            if (pg_num_rows($result) > 0):
                                while ($row = pg_fetch_assoc($result)):
                                    ...
                                    $imagenData = pg_fetch_result($imgResult, 0, 'imagen') ?? null;
                            ?>
                                    <tr>
                                        ...
                                        <td>
                                            <?php if ($imagenData): ?>
                                                <img src='data:image/jpeg;base64,<?= base64_encode(pg_unescape_bytea($imagenData)) ?>' width='100' height='100' />
                                            <?php else: ?>
                                                No image available
                                            <?php endif; ?>
                                        </td>
                                ...
                                <tr>
                                    <td colspan="6" style="text-align: center;">No properties available for this seller</td>
                                </tr>
                            <?php endif;
                            pg_close($conn); ?>
                        </tbody>
            ...
            <?php if (isset($_SESSION['suscripcion_renovada'])): ?>
                <script>
                    Swal.fire({
                        icon: 'success',
            ...
            <script>
                ...
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    const ya_tuvo_trial = <?= $ya_tuvo_trial ? 'true' : 'false' ?>;
                    ...
                    paypal.Buttons({
                        createOrder: function(data, actions) {
                            ...
                            if (!telefono || !/^\d{4}-\d{4}$/.test(telefono)) {
                                return Swal.fire({
                                    icon: 'warning',
                                    title: 'Error',
                                    text: 'Please enter a valid phone number in the format XXXX-XXXX.'
                                }).then(() => Promise.reject());
                            }
                            if (plan_id == 4 && ya_tuvo_trial) {
                                return Swal.fire({
                                    icon: 'warning',
                                    title: 'Error',
                                    text: 'The trial plan can only be used once. Please select a paid plan.'
                                }).then(() => Promise.reject());
                            }
                            return actions.order.create({
                                purchase_units: [{
                                    description: 'HomeSafe subscription renewal',
                                    amount: {
                                        currency_code: 'USD',
                                        value: precios_planes[plan_id]
                                    }
                                }]
                            });
                        },
                        onApprove: function(data, actions) {
                            return actions.order.capture().then(function(details) {
                                document.getElementById('paypal_transaction_id').value = details.purchase_units[0].payments.captures[0].id;
                        ...
                        onError: function(err) {
                            Swal.fire({
                                icon: 'error',
                                title: 'Error',
                                text: 'There was a problem with the PayPal payment. Please try again.'
                            });
                        }
