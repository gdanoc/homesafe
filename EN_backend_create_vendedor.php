...
if (!$conn) {
    echo '<p><script>swal({
        ...
        window.location = "EN_frontend_create_vendedor.php";
    });</script></p>';
    exit;
}
...
if (isset($_POST['submit'])) {
        ...
        if ($count >= 5) {
            echo '
                ...
                <script>
                    swal({
                        title: "Limit reached",
                        text: "Your HomeSafe Lite plan only allows registering five properties.",
                        icon: "warning",
                        button: "Close"
                    }).then(() => { window.location.href = "EN_dashboard_vendedor.php"; });
                </script>
                </body>
                </html>
                ';
    ...
    if ($plan_id === 2) {
        ...
        <script>
            swal({
                title: "Limit reached",
                text: "Your HomeSafe Plus plan only allows registering up to ten properties.",
                icon: "warning",
                button: "Close"
            }).then(() => { window.location.href = "EN_dashboard_vendedor.php"; });
        </script>
    ...
    if ($latitud === null || $longitud === null) {
        echo '<p><script>swal({
            title: "Error",
            text: "You must select a location on the map",
            icon: "warning",
            button: "Close",
        }).then(function() {
            window.location = "EN_frontend_create_vendedor.php";
    ...
    <script>
        swal({
            title: "Room limit",
            text: "Your HomeSafe Plus plan only allows adding up to 3 rooms per property.",
            icon: "warning",
            button: "Close"
        }).then(() => { window.history.back(); });
    </script>
    ...
    foreach ($room_names as $index => $room_name) {
        ...
        $room_type = $room_types[$index] ?? 'Other';
