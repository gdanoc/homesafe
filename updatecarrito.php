...
if (!isset($_POST['cantidad']) || !is_array($_POST['cantidad'])) {
    echo '<script>
        swal({
            title: "Error",
            text: "Datos de cantidades no válidos",
            icon: "warning",
            button: "Cerrar",
        }).then(function() {
...
foreach ($_POST['cantidad'] as $producto_id => $cantidad) {
    ...
    if (!$result) {
        $errores[] = "Error al actualizar producto ID $producto_id";
    } else {
        $actualizados++;
    }
}
...
if (count($errores) > 0) {
    ...
    header("Location: mostrarcarrito.php?update=error&msg=" . urlencode($mensaje));
    exit();
} else {
    header("Location: mostrarcarrito.php?update=success&count=$actualizados");
    exit();
}
