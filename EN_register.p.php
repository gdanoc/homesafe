...
if (!preg_match('/[A-Z]/', $password)) {
    echo "<script>alert('The password must contain at least one uppercase letter.'); window.location='EN_index.php';</script>";
    exit();
}


if (!preg_match('/[a-z]/', $password)) {
    echo "<script>alert('The password must contain at least one lowercase letter.'); window.location='EN_index.php';</script>";
    exit();
}


if (!preg_match('/[0-9]/', $password)) {
    echo "<script>alert('The password must contain at least one number'); window.location='EN_index.php';</script>";
    exit();
}


if (!preg_match('/[\W_]/', $password)) { 
    echo "<script>alert('The password must contain at least one special character.'); window.location='EN_index.php';</script>";
    exit();
}
...
if ($result && pg_num_rows($result) > 0) {
    echo "<script>
        alert('This email address is already registered or has a pending verification.');
        window.location='EN_index.php';
    </script>";
    exit();
}
...
try {
    ...
    $mail->isHTML(true);
    $mail->Subject = 'Verification code for HomeSafe';
    $mail->Body = "
    <!DOCTYPE html>
    <html lang='es'>
    <head>
        ...
        <title>Email Verification - HomeSafe</title>
        <style>
            body {
    ...
    <body>
        <div class='container'>
            ...
            <div class='content'>
                ...
                <p class='message'>Thank you for registering with HomeSafe. To complete your registration, we need to verify your email address.</p>
                ...
                <div class='token-section'>
                    ...
                    <p class='expiration-info'>This code will expire in 24 hours for security reasons.</p>
                </div>
                <p class='ignore-message'>If you did not request this registration, you can ignore this email.</p>
            </div>
            <div class='footer'>
    ...
    include 'EN_verify_token_modal.php';
} catch (Exception $e) {
    echo "<script>
        alert('Error sending verification email: " . addslashes($e->getMessage()) . "');
        window.location='EN_index.php';
    </script>";
    exit();
}
